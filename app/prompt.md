...
# 冒頭説明文
「前提の計算式」ブロックの上に、以下の説明ブロック（同幅）を表示して。

```
**What is this?**

最低生活保障(BI)、累進課税(収入逓減率α)、世帯構成によるコスト補正(β)を組み合わせた制度のシミュレーションができます。
Step.1 α, βを設定してください。αの効果は右のグラフで確認できます。
    従来制度から財政収支が変わらないように残りのパラメータ(γ)が自動的に計算されます。
Step.2 世帯員情報を入力してください。設定したパラメータでその世帯が受け取れる金額が計算されます。

```

# 計算式説明
"計算式"の見出しの右横にヘルプボタンを設置し、ホバーまたはタップで以下の説明文が表示されるようにして。
...
# 世帯員情報の複数世帯対応
「世帯員情報」に複数世帯の情報を入れられるよう、以下のように修正して。
1. 見出しを「世帯情報」に変更する。
2. レイアウトを以下のように「世帯」と「メンバー」（「世帯員」から名称変更）で異なるインデントorブロックになるようにする。
```
世帯情報
　世帯1
　　メンバー1
...
3. POSTリクエストは以下のようなフォーマットにする。以下で明示していない項目は従来と同様。
```json
{
    "世帯一覧": {
        "世帯1": {
...
4. 受け取ったレスポンスから世帯ごとに従来の計算を行い、"合計 (A)+(B)", "最低生活保障 (A)", "余剰額 (B)"を表示する。